...
Bool_t R3BLosReader::Init(ext_data_struct_info *a_struct_info)
{
	...
	for(UInt_t i = 0; i < NUM_LOS_CHANNELS; i++)
    {
...
*/
  ...
  cout<<"LOS READER, HERE WE GO"<<endl;
...
*/ 
 ...
 // First, we prepare time arrays for VFTX
  ...
  for ( int k=0;k<NUM_LOS_CHANNELS;k++)  // i=0,1,2,3...NUM_LOS_CHANNELS-1
   {	          
	uint32_t fine_time_VFTX[k]={1000000};     // first set unreasonably high values
    uint32_t coarse_times_VFTX[k]={1000000};
   }   
    ...
    for (int i=0;i<numChannels;i++)  // i=0,1,2,3...NUM_LOS_CHANNELS-1
	{
		...
		uint32_t nextChannelStart=data->LOS[d].VTFME[i];  // index in v for first item of next channel
   		...
   		for (int j=curChannelStart;j<nextChannelStart;j++){
            ...
            uint32_t fine_times_temp=data->LOS[d].VTFv[j];     // VFTX fine time
            uint32_t coarse_times_temp=data->LOS[d].VTCv[j];   // VFTX coarse time
 ...
 // for ( int k=0;k<NUM_LOS_CHANNELS;k++)  // i=0,1,2,3...NUM_LOS_CHANNELS-1
  // {	          
	 uint32_t fine_time_TAMEX_L[8]={1000000};     // first set unreasonably high values
     uint32_t coarse_times_TAMEX_L[8]={1000000};     
     uint32_t fine_time_TAMEX_T[8]={1000000};     
  ...
  // }
	  ...
	  uint32_t curChannelStartL=0;     // index in v for first item of current channel
		...
		for (int i=0;i<numChannelsL;i++)  // i=0,1,2,3...NUM_LOS_CHANNELS-1
		{
			...
			uint32_t nextChannelStartL=data->LOS[d].TTFLME[i]; // index in v for first item of next channel
             ...
             fine_time_TAMEX_L[i]=data->LOS[d].TTFLv[j];
             fine_time_TAMEX_T[i]=data->LOS[d].TTFTv[j];
             coarse_times_TAMEX_L[i]=data->LOS[d].TTCLv[j];
             coarse_times_TAMEX_T[i]=data->LOS[d].TTCTv[j];
      ...
      for ( int k=0;k<NUM_LOS_CHANNELS;k++)  // i=0,1,2,3...NUM_LOS_CHANNELS-1
      {          
        uint32_t fine_times[0]=fine_times_VFTX[k];
        uint32_t fine_times[1]=fine_time_TAMEX_L[k];
        uint32_t fine_times[2]=fine_time_TAMEX_T[k];
        uint32_t coarse_times[0]=coarse_times_VFTX[k];
        uint32_t coarse_times[1]=coarse_time_TAMEX_L[k];
        uint32_t coarse_times[2]=coarse_time_TAMEX_T[k];        
...
cout<<"R3BReader fMapped array: "<<"channel= "<<k<<"; Type= "<<itype<<"; Fine time= "<<fine_times[itype]<<"; Coarse time= "<<coarse_times[itype]<<endl;
