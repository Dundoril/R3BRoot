...
//  R3BFi1Mapped2Cal tjo;

  FairRuntimeDb* rtdb1 = run->GetRuntimeDb();
  Bool_t kParameterMerged = kTRUE;
  FairParRootFileIo* parOut2 = new FairParRootFileIo(kParameterMerged);
  parOut2->open("tcal_fi1_test012.root");
  rtdb1->setOutput(parOut2);

  rtdb1->print();

  //rtdb1->addRun(RunId);
  //rtdb1->getContainer("Fi1TCalPar");
  //rtdb1->setInputVersion(RunId, (char*)"Fi1TCalPar", 1, 1);
  ...
  auto fi1Calibrator = new R3BBunchedFiberMapped2CalPar("Fi1", 1);
  fi1Calibrator->SetUpdateRate(updateRate);
  fi1Calibrator->SetMinStats(minStats);
  run->AddTask(fi1Calibrator);
