...
void calib_fiber5(Int_t RunId = 67)
{
...
//  TString filename = "/d/land2/kvi2018/lmd/run"+runNumber+"*.lmd";
  ...
  TString outputFileName = "/d/land2/kvi2018/rootfiles/run"+runNumber+"_tcal.root";
...
#define RUN_ONLINE
  ...
  parOut2->open("tcal_fi5_"+runNumber+".root");
  ...
  auto fi5Calibrator = new R3BBunchedFiberMapped2CalPar("Fi5", 1,
      R3BBunchedFiberMapped2CalPar::CTDC);
  fi5Calibrator->SetUpdateRate(updateRate);
  fi5Calibrator->SetMinStats(minStats);
  run->AddTask(fi5Calibrator);
