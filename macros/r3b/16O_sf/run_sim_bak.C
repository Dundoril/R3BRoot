void run_sim()
{
    ...
    Bool_t magnet = kTRUE;
    Float_t fieldScale = -0.9; // 500 AMeV;
...
// 
    // Target
    Double_t distanceZeroTarget = 10.0;
    ...
    // Fi3a detector
    Double_t distanceZeroDet = 1.0;
    Double_t fi3Distance = 7.0;
    Double_t distanceZeroMagnet = 3.0;
    
    run->AddModule(new R3BFi3a("fi3a_16O.geo.root", {fi3Distance, 0.00, distanceZeroDet}, {"" ,90.,90.,0.0}));

    // Fi3b detector

    run->AddModule(new R3BFi3b("fi3b_16O.geo.root", {-fi3Distance, 0.00, distanceZeroDet}, {"" ,90.,90.,0.0}));

    // Fi7 detector
    Double_t fi7_8Distance = 29.0;
    Double_t fi7_8DistanceMagnetDetectors = 5.0;
    ...
    run->AddModule(new R3BFi7("fi7_16O.geo.root",fi7X, 0.0, fi7Z, -18.));
    ...
    run->AddModule(new R3BFi8("fi8_16O.geo.root",fi8X, 0., fi8Z, -18.));

    // Fi10 detector
    Double_t fi10_11Distance = 31.0;
    Double_t fi10_11DistanceMagnetDetectors = 7.0;
    ...
    run->AddModule(new R3BFi11("fi11_16O.geo.root", {fi11X, 0.0, fi11Z }, {"" ,90,90.0-18.0,0.0}));
    ...
    // dTof
    Double_t dTof_DistanceMagnetDetectors = 8.0;
    Double_t dTof_offset = 9.0;
    Double_t dTofX;
    Double_t dTofZ;
    dTofX = -(TMath::Sin(TMath::DegToRad()*18.0))*dTof_DistanceMagnetDetectors                      -  TMath::Cos(TMath::DegToRad()*18.0)*dTof_offset;
    dTofZ =  (TMath::Cos(TMath::DegToRad()*18.0))*dTof_DistanceMagnetDetectors + distanceZeroMagnet -  TMath::Cos(TMath::DegToRad()*18.0)*dTof_offset;
    ...
    if (generator.CompareTo("box") == 0)
    {
        ...
        FairIonGenerator* boxGen = new FairIonGenerator(8, 16, 8, 1, 0., 0., 1.08696875, 0, 0., -300.);
