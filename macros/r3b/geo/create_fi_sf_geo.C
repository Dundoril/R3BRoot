...
void create_fi_sf_geo(const char* geoTag)
{
  ...
  TString geoFileName = geoPath + "/geometry/fi_sf_";
  ...
  TGeoVolume* pGFILog = new TGeoVolume("FISFLog",pGFITube, pMed35);

  TGeoShape *pGFITubeActive = new TGeoTube("GFITubeActive", 0, (fiber_thickness * .97)/2, detector_height-0.0001);

  TGeoVolume* pGFILogActive = new TGeoVolume("FISF1Log",pGFITubeActive,pMed35);
