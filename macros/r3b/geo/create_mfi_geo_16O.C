...
void create_mfi_geo(const char* geoTag)
{
  ...
  dx = 100.000;	//half width of box for mfi volumes
  dy = 50.00000;
  dz = 0.050000;
...
//  pMFILogWorld->AddNode(plogicBackCladding, 0, pMatrix10);
  ...
  dx = 20.000000; //half width of active volume
  ...
  pMFILog->SetLineColor(kYellow);
  pMFILogWorld->AddNode(pMFILog, 0, pMatrix12); //put here the active volume of mfi
  pMFILogWorld->AddNode(pMFILog, 1, pMatrix14); //put here the active volume of mfi
