...
void create_fi3_geo(const char* geoTag)
{
  ...
  TString geoFileName = geoPath + "/geometry/fi3_";
  ...
  Float_t detector_width = 10.000000;
  Float_t detector_height = 10.000000;
  Float_t fiber_thickness = 0.020000; 
  Int_t fiber_nbr = 512;
  Float_t dead_layer = 0.9;
  
  TGeoShape *pGFIBox = new TGeoBBox("GFIBox", fiber_thickness/2, fiber_thickness/2 ,detector_height/2);
  
  TGeoVolume* pGFILog = new TGeoVolume("FI4Log",pGFIBox, pMed35);

  TGeoShape *pGFIBoxActive = new TGeoBBox("GFIBoxActive", (fiber_thickness * dead_layer)/2, detector_height/2-0.0001, (fiber_thickness * dead_layer)/2);

  TGeoVolume* pGFILogActive = new TGeoVolume("FI41Log",pGFIBoxActive,pMed35);
  ...
  for(int fiber_id = 0; fiber_id < fiber_nbr ; fiber_id++)
  {
	pGFILogWorld->AddNode(pGFILog, real_fiber_id++, 
		new TGeoCombiTrans("", 
			-detector_width / 2 + (fiber_id + .5) * fiber_thickness, // sf: why + .5
			0,
			-(fiber_thickness) / 2,
			pMatrixBox
		)
