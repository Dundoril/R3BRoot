...
void create_fi8_geo(const char* geoTag)
{
  ...
  TString geoFileName = geoPath + "/geometry/fi8_";
  ...
  TGeoVolume*   pGFILogWorld = new TGeoVolumeAssembly("FI8LogWorld");
  pGFILogWorld->SetVisLeaves(kTRUE);
  
  // Global positioning
  pWorld->AddNode(pGFILogWorld, 0, pMatrix2);
  
  Float_t detector_height = 55.000000 ;
  Float_t fiber_thickness = 0.10000 ; 
  Int_t fiber_nbr = 500;
  Float_t dead_layer = 0.9;
  Float_t air_layer = 0.05; // relative to fiber_thickness
  ...
  TGeoVolume* pGFILog = new TGeoVolume("FI8Log",pGFIBox, pMed35);
  ...
  TGeoVolume* pGFILogActive = new TGeoVolume("FI81Log",pGFIBoxActive,pMed35);
  ...
  pGFILogActive->SetLineColor(kBlue);
  ...
  for(int fiber_id = 0; fiber_id < fiber_nbr ; fiber_id++)
  {
	...
	// pGFILogWorld->AddNode(pGFILogActive, real_fiber_id++, 
	// 	new TGeoCombiTrans("", 
	// 		-detector_width / 2  +(fiber_id + (fiber_id * air_layer)) * fiber_thickness, // sf: why + .5
	// 		0,
	// 		-(fiber_thickness) / 2 ,
	// 		pMatrixBox
	// 	)
