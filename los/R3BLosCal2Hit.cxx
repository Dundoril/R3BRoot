...
//        cout<<"SciType "<<iSciType<<endl;
     ...
     // min,max,Nbins for X and Y spectra
       Double_t fhXmin = -10.; //-5
       Double_t fhXmax = 10.; // 5
...
//    cout<<"TAMEX T 1-8 "<<time_t_T<<", "<<time_lt_T<<", "<<time_l_T<<", "<<time_lb_T<<", "<<time_b_T<<", "<<time_rb_T<<", "<<time_r_T<<", "<<time_rt_T<<endl;  
 ...
 // For octagonal detector, two runs are  made - once ref signal is put to channel 1 and once to channel 8.
  ...
  // This is for 12C beam:
         // if((tot_ver+tot_hor+tot_45+tot_135)/4. > 52. && (tot_ver+tot_hor+tot_45+tot_135)/4. < 62.) Igood_event = 1;
...
// if octagon  
  ...
  /*      
    ...
    */               
          ...
          yT_cm = (time_t_L+time_lt_L)/2.-(time_b_L+time_rb_L)/2.;
  ...
  // New coordinate system since KVI (transformation for 22.5 deg):         
          ...
          y_cm = (time_t_V+time_lt_V)/2.-(time_b_V+time_rb_V)/2.;
...
*/
           xToT=((totl+totlb)/2.-(totr+totrt)/2.)/((totl+totlb+totr+totrt)/4.);
...
*/
          xreal=((totl_corr+totlb_corr)/2.-(totr_corr+totrt_corr)/2.)/((totl_corr+totlb_corr+totr_corr+totrt_corr)/4.);
 ...
 // Fill the spectra:
  ...
  // x,y from MCFD:
        //  fhXY->Fill((time_t_V+time_lt_V)/2.-(time_rb_V+time_b_V)/2.,(time_l_V+time_lb_V)/2.-(time_r_V+time_rt_V)/2.);
          
          fhXY->Fill(x_cm,y_cm);         
          //fhXY1->Fill(x1_cm,y1_cm);
            fhXY1->Fill((x1_cm*cos(3.141593/8.) + y1_cm*sin(3.141593/8.)),(y1_cm*cos(3.141593/8.) - x1_cm*sin(3.141593/8.) ));
          //fhXY2->Fill(x2_cm,y2_cm); 
            fhXY2->Fill(x2_cm*cos(3.141593/8.) - y2_cm*sin(3.141593/8.),x2_cm*sin(3.141593/8.) + y2_cm*cos(3.141593/8.)); 
      // Average of three:         
  
          fhXYproj->Fill(((xreal+x_cm+xT_cm)/3.),(yreal+y_cm+yT_cm)/3.);             
          //fhXYproj->Fill((x1T_cm*cos(3.141593/8.) + y1T_cm*sin(3.141593/8.)),(y1T_cm*cos(3.141593/8.) - x1T_cm*sin(3.141593/8.) ));
 // Mean value between MCFD and ToT:  
          fhXYmean->Fill(xmean_cm,ymean_cm);         
      //    fhXYmean->Fill(x2T_cm*cos(3.141593/8.) - y2T_cm*sin(3.141593/8.),x2T_cm*sin(3.141593/8.) + y2T_cm*cos(3.141593/8.));
  ...
  // x,y from TAMEX:	           
         // fhXYT->Fill(((time_r_L+time_rt_L)/2.-(time_l_L+time_lb_L)/2.)*(1.),((time_rb_L+time_b_L)/2.-(time_t_L+time_lt_L)/2.)*(1.));
          
          fhXYT->Fill(xT_cm,yT_cm); 
          //fhXYT1->Fill(x1T_cm,y1T_cm);
          fhXYT1->Fill((x1T_cm*cos(3.141593/8.) + y1T_cm*sin(3.141593/8.)),(y1T_cm*cos(3.141593/8.) - x1T_cm*sin(3.141593/8.) ));
          //fhXYT2->Fill(x2T_cm,y2T_cm);
          fhXYT2->Fill(x2T_cm*cos(3.141593/8.) - y2T_cm*sin(3.141593/8.),x2T_cm*sin(3.141593/8.) + y2T_cm*cos(3.141593/8.));        
  // x,y from ToT:            
